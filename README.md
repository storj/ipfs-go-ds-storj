...
`$updateBloomFilter` is an option boolean that enables the bloom filter updater. If not set, the updater is disabled.
...
`STORJ_UPDATE_BLOOM_FILTER` enables the bloom filter updater, which listen to changes in the local database and updates the datastore bloom filter. The default value is false. It should be enabled when running multiple nodes attached to the same datastore. Only CockroachDB is supported.
